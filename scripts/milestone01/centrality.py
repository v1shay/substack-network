#!/usr/bin/env python3
"""
Compute RecommendationRank (and in-degree) on the recommendation graph.
Loads publications + recommendations from cartographer.db, builds a directed graph,
runs PageRank on recommendation edges, and outputs a top-N rankings table
(and optional CSV to data/).
...
Usage (from repo root, cartographer.db at repo root; or set CARTOGRAPHER_ROOT):
    ...
    python scripts/milestone01/centrality.py -o data/recommendation_rank.csv
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RecommendationRank on the recommendation graph (cartographer.db)."
    )
    parser.add_argument("--db", type=str, default=None, help="Path to cartographer.db (default: cwd/cartographer.db)")
    parser.add_argument("-n", type=int, default=50, metavar="N", help=f"Show and export top N by {RECOMMENDATION_RANK_NAME} (default: 50)")
    parser.add_argument("-o", "--output", type=str, default=None, help="Write rankings to this CSV (e.g. data/recommendation_rank.csv)")
    ...
    # Build rankings: (domain, name, recommendation_rank, in_degree, depth), sorted by rank desc
    rows = []
    ...
    # Print table
    print(f"Top {top_n} by {RECOMMENDATION_RANK_NAME} (nodes={G.number_of_nodes()}, edges={G.number_of_edges()})")
    print("Algorithm: PageRank over directed publication recommendation edges.\n")
    print(f"{'rank':<6} {'domain':<35} {'name':<30} {'recommendation_rank':<20} {'in_degree':<10} {'depth':<6}")
    print("-" * 102)
    for i, (domain, name, pr, deg, depth) in enumerate(top, 1):
        ...
        print(f"{i:<6} {domain_short:<35} {name_short:<30} {pr:<20.6f} {deg:<10} {depth_str:<6}")
    ...
    if out_csv:
        ...
        with open(out_csv, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["rank", "domain", "name", "recommendation_rank", "in_degree", "depth"])
            for i, (domain, name, pr, deg, depth) in enumerate(top, 1):
